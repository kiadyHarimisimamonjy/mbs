...
$openlast=CheckoutOpen::where('counter_id', $counter->id)->
where('created_at','<', $this->created_at)
->orderBy('created_at', 'desc')->first();
$debut=$openlast->checkoutclose->created_at;
          $canceled= CanceledPaiement::where('created_at','>',$debut)->
             where('counter_id',$this->counter_id)->get();
          $sumcanceled = array_sum(array_column(json_decode(json_encode($canceled), true),
          'montant'));
          //$sumcanceled=
          $paie= Paiement::where('created_at','>',$debut)->
       where('counter_id',$this->counter_id) ->get();
          $sumpaid = array_sum(array_column(json_decode(json_encode($paie), true),
          'montant'));
          $depense=ActionDepense::where('created_at','>',$debut)->
          ...
          $sumdepense = array_sum(array_column(json_decode(json_encode($depense), true),
          'montant'));
    $this->isclosed=1;
    $this->save();
